describe("Patient Workflow", () => {
  ...
  it("completes full patient workflow", async () => {
    try {
      ...
      await waitFor(element(by.id("risk-details-modal")))
        .toBeVisible()
        .withTimeout(5000);
      ...
      await waitFor(element(by.id("clinical-history-list")))
        .toBeVisible()
        .withTimeout(5000);
      ...
      await waitFor(element(by.text("Patient showing improvement")))
        .toBeVisible()
        .withTimeout(5000);
      ...
      await element(by.id("risk-notes-input")).typeText(
        "Increased risk due to recent lab results",
      );
      ...
      await waitFor(element(by.id("risk-score")))
        ...
        .withTimeout(5000);
      await waitFor(
        element(by.text("Increased risk due to recent lab results")),
      )
        .toBeVisible()
        .withTimeout(5000);
      ...
      await waitFor(element(by.id("lab-results-list")))
        .toBeVisible()
        .withTimeout(5000);
      ...
      await waitFor(element(by.id("export-success-message")))
        .toBeVisible()
        .withTimeout(5000);
    } catch (error) {
  ...
  it("validates user inputs", async () => {
    try {
      ...
      await element(by.id("save-risk-button")).tap();
      await waitFor(element(by.id("error-message")))
        .toHaveText("Risk score must be between 0 and 1")
        .withTimeout(5000);
      ...
      await element(by.id("save-note-button")).tap();
      await waitFor(element(by.id("error-message")))
        .toHaveText("Note cannot be empty")
        .withTimeout(5000);
    } catch (error) {
      console.error("Test failed:", error);
      throw error;
  ...
  it("handles offline mode", async () => {
    try {
      ...
      await waitFor(element(by.id("offline-warning")))
        .toBeVisible()
        .withTimeout(5000);

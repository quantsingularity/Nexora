    ...
    if (patientId === 'p001') {
      return {
        ...
        return {
            ...
            predictions: {
              risk: 0.85,
              top_features: ['multiple comorbidities', 'age > 55', 'lab_value_x abnormal'],
              cohort_size: 95,
...
const PatientDetailScreen = ({ route, navigation }) => {
  ...
  if (!details) {
    return (
      <ScreenWrapper style={GlobalStyles.centered}><Text style={Typography.body1}>Could not load patient details.</Text></ScreenWrapper>
    );
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContainer}>
        ...
        <Card style={styles.riskCard}>
          ...
          <Text style={styles.subText}>
            Confidence: [{`${(details.uncertainty.confidence_interval[0] * 100).toFixed(0)}%`} - {`${(details.uncertainty.confidence_interval[1] * 100).toFixed(0)}%`}]
          </Text>
        </Card>
        ...
        <Card>
          ...
          <LineChart
            data={shapData}
            width={Dimensions.get("window").width - Spacing.md * 2 - Spacing.md * 2} // Screen width - screen padding - card padding
            height={220}

...
const getPatientDetails = async (patientId) => {
  ...
  if (patientId === 'p001') {
    return {
      id: 'p001', name: 'John Doe', risk: 0.75,
      predictions: { risk: 0.75, top_features: ['age', 'previous_admissions', 'diabetes'], cohort_size: 120, shap_features: ['age', 'prev_adm', 'diabetes', 'htn', 'hf'], shap_values: [0.3, 0.25, 0.2, 0.15, 0.1] },
      explanations: { method: 'SHAP', values: [0.3, 0.25, 0.2, 0.15, 0.1] }, uncertainty: { confidence_interval: [0.65, 0.85] }, timeline: []
    };
  } else if (patientId === 'p003') {
      return {
          id: 'p003', name: 'Robert Johnson', risk: 0.85,
          predictions: { risk: 0.85, top_features: ['comorbidities', 'age', 'lab_value_x'], cohort_size: 95, shap_features: ['comorb', 'age', 'lab_x', 'med_y', 'prev_adm'], shap_values: [0.4, 0.2, 0.15, 0.05, 0.05] },
          explanations: { method: 'SHAP', values: [0.4, 0.2, 0.15, 0.05, 0.05] }, uncertainty: { confidence_interval: [0.78, 0.92] }, timeline: []
        };
  }
  return {
    id: patientId, name: 'Default Patient', risk: 0.55,
    predictions: { risk: 0.55, top_features: ['feature_a', 'feature_b', 'feature_c'], cohort_size: 150, shap_features: ['feat_a', 'feat_b', 'feat_c', 'feat_d', 'feat_e'], shap_values: [0.2, 0.15, 0.1, 0.05, 0.05] },
    explanations: { method: 'SHAP', values: [0.2, 0.15, 0.1, 0.05, 0.05] }, uncertainty: { confidence_interval: [0.45, 0.65] }, timeline: []
  };
};

...
class DeploymentValidator:
    ...
    def __init__(self, config_path: str, env: str, timeout: int = 300, 
                 rollback: bool = False, verbose: bool = False):
        """
    ...
    def run_all_validations(self) -> Dict:
        ...
        logger.info(f"Deployment validations completed with status: {self.results['status']}")
        return self.results
            ...
            try:
                ...
                for deployment in deployments:
                    if deployment.metadata.name == deployment_name:
                        ...
                        if deployment.metadata.labels and "version" in deployment.metadata.labels:
                            return deployment.metadata.labels["version"]
                        if deployment.metadata.annotations and "version" in deployment.metadata.annotations:
                            return deployment.metadata.annotations["version"]
                        
                        # Try to parse version from image
                        if (deployment.spec.template.spec.containers and 
    ...
    def validate_kubernetes(self) -> None:
        ...
        try:
            ...
            if k8s_config.get("validate_deployments", True):
                deployments_status, deployments_details = self._validate_k8s_deployments(
            ...
            if k8s_config.get("validate_pods", True):
                pods_status, pods_details = self._validate_k8s_pods(
                    core_v1, namespace
                )
    ...
    def _validate_k8s_pods(self, core_v1, namespace: str) -> Tuple[str, Dict]:
        ...
        try:
            ...
            return "ERROR", {
                "message": f"Error validating Kubernetes pods: {str(e)}"
            }
    ...
    def _get_pod_age(self, pod) -> str:
        ...
        if pod.metadata.creation_timestamp:
            age_seconds = (datetime.datetime.now(datetime.timezone.utc) - 
                          pod.metadata.creation_timestamp.replace(tzinfo=datetime.timezone.utc)).total_seconds()
    ...
    def validate_services(self) -> None:
        ...
        for service in services_config:
            ...
            if service_type == "http":
                ...
                status, details = self._validate_command(command, expected_exit_code, timeout)
                service_validations[service_name] = {
                    "status": status,
                    "type": "command",
                    "command": command,
    ...
    def _validate_http_service(
        self, url: str, method: str, expected_status: int, 
        timeout: int, headers: Dict, content_check: Optional[str]
    ) -> Tuple[str, Dict]:
        """Validate HTTP service"""
        if not REQUESTS_AVAILABLE:
            return "ERROR", {"message": "requests library not available for HTTP validation"}
        ...
        try:
            ...
            if content_check and status_check:
                if content_check in response.text:
                    ...
                    content_check_message = f"Response does not contain '{content_check}'"
            else:
            ...
            return "FAIL", {
                "message": f"Connection error when connecting to {url}"
            }
        except Exception as e:
            return "ERROR", {
    ...
    def _validate_tcp_service(self, host: str, port: int, timeout: int) -> Tuple[str, Dict]:
        """Validate TCP service"""
        try:
            start_time = time.time()
    ...
    def validate_api_endpoints(self) -> None:
        ...
        endpoints_config = self.config["environments"][self.env].get("api_endpoints", [])
        ...
        for endpoint in endpoints_config:
            ...
            status, details = self._validate_api_endpoint(
                url, method, headers, data, expected_status, timeout, content_check, schema_validation
            )
            endpoint_validations[endpoint_name] = {
                "status": status,
                "url": url,
    ...
    def _validate_api_endpoint(
        self, url: str, method: str, headers: Dict, data: Any,
        expected_status: int, timeout: int, content_check: Optional[str],
                    ...
                    if "required_fields" in schema_validation:
                        for field in schema_validation["required_fields"]:
                            if field not in response_json:
                                schema_errors.append(f"Required field '{field}' missing")
                    
                    # Check field types
                    if "field_types" in schema_validation:
                        for field, expected_type in schema_validation["field_types"].items():
                            if field in response_json:
                                # Check type
                                if expected_type == "string" and not isinstance(response_json[field], str):
                                    schema_errors.append(f"Field '{field}' should be a string")
                                elif expected_type == "number" and not isinstance(response_json[field], (int, float)):
                                    schema_errors.append(f"Field '{field}' should be a number")
                                elif expected_type == "boolean" and not isinstance(response_json[field], bool):
                                    schema_errors.append(f"Field '{field}' should be a boolean")
                                elif expected_type == "array" and not isinstance(response_json[field], list):
                                    schema_errors.append(f"Field '{field}' should be an array")
                                elif expected_type == "object" and not isinstance(response_json[field], dict):
                                    schema_errors.append(f"Field '{field}' should be an object")
                    
                    if schema_errors:
                        schema_check_result = False
                        schema_check_message = f"Schema validation failed: {', '.join(schema_errors)}"
                    else:
        ...
        for db in databases_config:
            ...
            if db_type == "postgresql":
                ...
                status, details = self._validate_postgresql(
                    host, port, database, user, password, ssl_mode, timeout, validation_query
                )
                database_validations[db_name] = {
                    "status": status,
                    "type": db_type,
                ...
                if uri:
                    ...
                    status, details = self._validate_mongodb(
                        host, port, database, user, password, timeout, validation_collection
                    )
                
                database_validations[db_name] = {
                    "status": status,
                    "type": db_type,
                    **({"uri": uri} if uri else {"host": host, "port": port, "database": database}),
                    **details
                }
    ...
    def _validate_postgresql(
        ...
        password: str, ssl_mode: str, timeout: int, validation_query: str
    ) -> Tuple[str, Dict]:
    ...
    def _validate_mongodb_uri(
        ...
        try:
            ...
            return "PASS", {
                ...
                **({"collection_validation": collection_validation} if collection_validation else {})
            }
    ...
    def _validate_mongodb(
        ...
        user: str, password: str, timeout: int, validation_collection: Optional[str]
    ) -> Tuple[str, Dict]:
    ...
    def validate_configurations(self) -> None:
        ...
        config_validations = self.config["environments"][self.env].get("config_validations", [])
        ...
        for validation in config_validations:
            ...
            logger.debug(f"Validating configuration: {validation_name} (type: {validation_type})")
            ...
            if validation_type == "file":
                ...
                status, details = self._validate_config_file(file_path, required_settings)
                configuration_results[validation_name] = {
                    "status": status,
                    "type": validation_type,
                    "file_path": file_path,
        ...
        try:
            ...
            for setting in required_settings:
                ...
                if expected_type:
                    ...
                    if not type_valid:
                        invalid_settings.append(f"{path} (expected type: {expected_type})")
                        continue
                
                # Check value if specified
                if expected_value is not None and actual_value != expected_value:
                    invalid_settings.append(f"{path} (expected: {expected_value}, actual: {actual_value})")
                ...
                for validation_name, validation_data in validations.items():
                    if isinstance(validation_data, dict) and "status" in validation_data:
                        ...
                        for key, value in validation_data.items():
                            if key not in ["status", "message"] and not isinstance(value, (dict, list)):
    ...
    def _generate_html_report(self) -> str:
        ...
        html.append("    .validation { margin-bottom: 10px; border: 1px solid #ddd; padding: 10px; border-radius: 5px; }")
        html.append("    .validation-header { display: flex; justify-content: space-between; }")
        ...
        html.append("    .toggle-btn { cursor: pointer; background: none; border: none; font-size: 16px; }")
        ...
        html.append(f"    <h1>Nexora Deployment Validation Report - {self.env.upper()}</h1>")
        html.append(f"    <p><strong>Date:</strong> {self.results['timestamp']}</p>")
        html.append(f"    <p><strong>Status:</strong> <span class='status status-{self.results['status']}'>{self.results['status']}</span></p>")
        html.append("  </div>")
        ...
        if "deployment_info" in self.results:
            ...
            html.append("    <table>")
            html.append(f"      <tr><th>Name</th><td>{info.get('name', 'unknown')}</td></tr>")
            html.append(f"      <tr><th>Environment</th><td>{info.get('environment', 'unknown')}</td></tr>")
            html.append(f"      <tr><th>Expected Version</th><td>{info.get('expected_version', 'unknown')}</td></tr>")
            html.append(f"      <tr><th>Actual Version</th><td>{info.get('actual_version', 'unknown')}</td></tr>")
            html.append(f"      <tr><th>Timestamp</th><td>{info.get('timestamp', 'unknown')}</td></tr>")
            html.append("    </table>")
            html.append("  </div>")
        ...
        html.append("    <table>")
        html.append("      <tr><th>Total Validations</th><td>" + str(summary['total_validations']) + "</td></tr>")
        for status, count in summary["status_counts"].items():
            ...
            if isinstance(validations, dict):
                for validation_name, validation_data in validations.items():
                    if isinstance(validation_data, dict) and "status" in validation_data:
                        ...
                        html.append(f"        <span class='status status-{status}'>{status}</span>")
                        html.append(f"      </div>")
                        html.append(f"      <div class='validation-message'>{message}</div>")
                        
                        # Add toggle button for details
                        html.append(f"      <button id='validation-{validation_id}-btn' class='toggle-btn' onclick=\"toggleDetails('validation-{validation_id}-details')\">▼</button>")
                        
                        # Add additional details
                        html.append(f"      <div id='validation-{validation_id}-details' class='validation-details'>")
                        for key, value in validation_data.items():
                            if key not in ["status", "message"]:
                                if isinstance(value, (dict, list)):
                                    html.append(f"        <div><strong>{key}:</strong> <pre>{json.dumps(value, indent=2)}</pre></div>")
                                else:
        ...
        if "rollback" in self.results:
            ...
            html.append("    <table>")
            html.append(f"      <tr><th>Status</th><td><span class='status status-{rollback.get('status', 'UNKNOWN')}'>{rollback.get('status', 'UNKNOWN')}</span></td></tr>")
            html.append(f"      <tr><th>Message</th><td>{rollback.get('message', 'No message')}</td></tr>")
            html.append("    </table>")
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        help="Path to output report (default: stdout)"
    )
    parser.add_argument(
        "--format",

...
class HealthCheck:
    ...
    def __init__(self, config_path: str, env: str, threshold: int = 80, verbose: bool = False):
        """
    ...
    def run_all_checks(self) -> Dict:
        ...
        # Kubernetes checks (if configured)
    ...
    def check_system_resources(self) -> None:
        ...
        if PSUTIL_AVAILABLE:
            ...
            cpu_status = "PASS" if cpu_percent < system_config.get("max_cpu_load", 90) else "FAIL"
            system_checks["cpu"] = {
                "status": cpu_status,
                "value": f"{cpu_percent}%",
        ...
        if PSUTIL_AVAILABLE:
            ...
            memory_available_status = "PASS" if memory_available_mb > min_memory_mb else "FAIL"
        ...
        # Check system load (Unix-like systems only)
    ...
    def check_services(self) -> None:
        ...
        for service in services_config:
            ...
            if service_type == "process":
                ...
                status, details = self._run_command(command, expected_exit_code, timeout)
                service_checks[service_name] = {
                    "status": status,
                    "type": "command",
                    "command": command,
    ...
    def check_kubernetes(self) -> None:
        ...
        try:
            ...
            if k8s_config.get("check_deployments", True):
                deployments_status, deployments_details = self._check_k8s_deployments(apps_v1, namespace)
            ...
            if k8s_config.get("check_services", True):
                services_status, services_details = self._check_k8s_services(core_v1, namespace)
                k8s_checks["services"] = {
    ...
    def _check_http_endpoint(
        ...
        """Check HTTP endpoint"""
        if not REQUESTS_AVAILABLE:
            return "ERROR", {"message": "requests library not available for HTTP check"}
    ...
    def _check_postgresql(
        ...
        password: str, ssl_mode: str, timeout: int
    ) -> Tuple[str, Dict]:
    ...
    def _check_mongodb_uri(self, uri: str, timeout: int) -> Tuple[str, Dict]:
        ...
        try:
            ...
            return "ERROR", {
                "message": f"Error checking MongoDB database: {str(e)}"
            }
    
    def _check_mongodb(
        self, host: str, port: int, database: str, 
        user: str, password: str, timeout: int
    ) -> Tuple[str, Dict]:
    ...
    def _check_redis(
        ...
        try:
            ...
            return "PASS", {
                ...
                "mode": "cluster" if info.get("cluster_enabled", 0) else "standalone"
            }
            ...
            return "ERROR", {
                "message": f"Error checking Redis database: {str(e)}"
            }
    ...
    def _check_k8s_nodes(self, core_v1) -> Tuple[str, Dict]:
    ...
    def _check_k8s_pods(self, core_v1, namespace: str) -> Tuple[str, Dict]:
        ...
        try:
            ...
            return "ERROR", {
                "message": f"Error checking Kubernetes pods: {str(e)}"
            }
    ...
    def _check_k8s_deployments(self, apps_v1, namespace: str) -> Tuple[str, Dict]:
        ...
        try:
            ...
            if not_ready_deployments:
                return "FAIL", {
                    ...
                    "ready_deployments": len(deployments) - len(not_ready_deployments)
                }
            else:
                return "PASS", {
    ...
    def _check_k8s_services(self, core_v1, namespace: str) -> Tuple[str, Dict]:
    ...
    def _calculate_overall_status(self) -> None:
        ...
        # Count statuses from all checks
    ...
    def _generate_html_report(self) -> str:
        ...
        html.append("    .check { margin-bottom: 10px; border: 1px solid #ddd; padding: 10px; border-radius: 5px; }")
        html.append("    .check-header { display: flex; justify-content: space-between; }")
        ...
        html.append(f"    <h1>Nexora Environment Health Check Report - {self.env.upper()}</h1>")
        ...
        html.append("    <table>")
        html.append("      <tr><th>Total Checks</th><td>" + str(summary['total_checks']) + "</td></tr>")
        for status, count in summary["status_counts"].items():
            ...
            if isinstance(checks, dict):
                for check_name, check_data in checks.items():
                    if isinstance(check_data, dict) and "status" in check_data:
                        ...
                        html.append(f"      <button id='check-{check_id}-btn' class='toggle-btn' onclick=\"toggleDetails('check-{check_id}-details')\">▼</button>")
                        
                        # Add additional details
                        html.append(f"      <div id='check-{check_id}-details' class='check-details'>")
                        for key, value in check_data.items():

...
class PHIDeidentifier:
    ...
    def __init__(self, config: DeidentificationConfig = None):
        ...
        self.global_date_shift = np.random.randint(-self.config.max_date_shift_days, 
                                                  self.config.max_date_shift_days)
        
    def deidentify_dataframe(self, df: pd.DataFrame, 
                            patient_id_col: Optional[str] = None,
                            phi_cols: Optional[List[str]] = None) -> pd.DataFrame:
        """
        ...
        for col in phi_cols:
            ...
            if self._is_date_column(result[col]):
                if self.config.shift_dates:
                    result[col] = self._shift_dates(result[col], result.get(patient_id_col))
                ...
                if (self._is_mrn_column(col) and self.config.remove_mrns) or \
                   (self._is_ssn_column(col) and self.config.remove_ssn) or \
    ...
    def _deidentify_patient_resource(self, resource: Dict[str, Any]) -> None:
        ...
        if 'birthDate' in resource and self.config.remove_dates_of_birth:
            ...
            if self.config.shift_dates:
                ...
                birth_year = resource['birthDate'][:4] if len(resource['birthDate']) >= 4 else None
                if birth_year and self._calculate_age(resource['birthDate']) >= self.config.age_threshold:
                    birth_year = str(int(birth_year) - int(birth_year) % 10)  # Truncate to decade
        ...
        if 'subject' in resource and 'reference' in resource['subject']:
            if self.config.hash_patient_ids:
                ...
                if ref.startswith('Patient/'):
                    ...
                    resource['subject']['reference'] = f"Patient/{self._hash_identifier(patient_id)}"
    ...
    def _extract_patient_id_from_resource(self, resource: Dict[str, Any]) -> Optional[str]:
    ...
    def _detect_phi_columns(self, df: pd.DataFrame) -> List[str]:
        ...
        """
        phi_patterns = [
            r'(?i)name', r'(?i)address', r'(?i)birth', r'(?i)dob', 
            r'(?i)phone', r'(?i)email', r'(?i)ssn', r'(?i)social', 
            r'(?i)mrn', r'(?i)medical.?record', r'(?i)patient.?id',
            r'(?i)zip', r'(?i)postal', r'(?i)city', r'(?i)state',
            r'(?i)date', r'(?i)age', r'(?i)identifier', r'(?i)license',
            r'(?i)account', r'(?i)url', r'(?i)ip.?address', r'(?i)device'
        ]
    ...
    def _is_date_column(self, series: pd.Series) -> bool:
        ...
        if series.dtype == 'object':
            ...
            for val in sample:
                if isinstance(val, str) and any(re.search(pattern, val) for pattern in date_patterns):
                    return True
    ...
    def _is_age_column(self, col_name: str) -> bool:
        ...
        age_patterns = [r'(?i)^age$', r'(?i)age[_\s]', r'(?i)[_\s]age$', 
                        r'(?i)patient[_\s]age', r'(?i)age[_\s]years']
    ...
    def _is_address_column(self, col_name: str) -> bool:
        ...
        address_patterns = [r'(?i)address', r'(?i)street', r'(?i)city', 
                           r'(?i)state', r'(?i)zip', r'(?i)postal']
    ...
    def _is_name_column(self, col_name: str) -> bool:
        ...
        name_patterns = [r'(?i)name', r'(?i)first[_\s]name', r'(?i)last[_\s]name', 
                        r'(?i)middle[_\s]name', r'(?i)patient[_\s]name']
    ...
    def _is_contact_info_column(self, col_name: str) -> bool:
        ...
        contact_patterns = [r'(?i)phone', r'(?i)email', r'(?i)fax', 
                           r'(?i)contact', r'(?i)mobile', r'(?i)cell']
    ...
    def _is_id_column(self, col_name: str) -> bool:
        ...
        id_patterns = [r'(?i)id$', r'(?i)identifier', r'(?i)number', 
                      r'(?i)code', r'(?i)key']
    ...
    def _is_mrn_column(self, col_name: str) -> bool:
        ...
        mrn_patterns = [r'(?i)mrn', r'(?i)medical[_\s]record', 
                       r'(?i)record[_\s]number']
    ...
    def _is_ssn_column(self, col_name: str) -> bool:
        ...
        ssn_patterns = [r'(?i)ssn', r'(?i)social[_\s]security', 
                       r'(?i)tax[_\s]id']
    ...
    def _is_device_id_column(self, col_name: str) -> bool:
        ...
        device_patterns = [r'(?i)device[_\s]id', r'(?i)serial[_\s]number', 
                          r'(?i)imei', r'(?i)uuid']
    ...
    def _shift_dates(self, date_series: pd.Series, 
                    patient_id_series: Optional[pd.Series] = None) -> pd.Series:
        """
        ...
        if self.config.date_shift_strategy == 'patient' and patient_id_series is not None:
    ...
    def _shift_date_string(self, date_str: str, 
    ...
    def _truncate_ages(self, age_series: pd.Series) -> pd.Series:
        ...
        return age_series.apply(
            lambda x: f"{self.config.age_threshold}+" 
    ...
    def _apply_k_anonymity(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        quasi_identifiers = []
        for col in df.columns:
            if self._is_age_column(col) or 'gender' in col.lower() or 'race' in col.lower() or 'ethnicity' in col.lower():
        ...
        small_groups = group_counts[group_counts['count'] < self.config.k_anonymity_threshold]

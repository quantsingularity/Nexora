...
class PHIDetector:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            'mrn': re.compile(r'\b(MRN|mrn|Medical Record Number|medical record number)[:# ]?\s*\d+\b'),
    ...
    def detect_phi_in_dataframe(self, df: pd.DataFrame, 
                               sample_size: int = 100) -> Dict[str, Dict[str, int]]:
        """
        ...
        for col in sample_df.columns:
            ...
            if not pd.api.types.is_string_dtype(sample_df[col]) and not pd.api.types.is_object_dtype(sample_df[col]):
                continue
    ...
    def identify_phi_columns(self, df: pd.DataFrame, 
                            threshold: float = 0.1) -> Dict[str, Set[str]]:
        """
    ...
    def generate_phi_report(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in all_phi_columns:
            combined_results[col] = {
                ...
                'risk_level': 'high' if col in phi_columns else 'medium' if col in name_based_phi else 'low'
            }
        ...
        summary = {
            ...
            'high_risk_columns': sum(1 for info in combined_results.values() if info['risk_level'] == 'high'),
            'medium_risk_columns': sum(1 for info in combined_results.values() if info['risk_level'] == 'medium'),
            'phi_types_detected': set().union(*[info['content_detection'] for info in combined_results.values() if info['content_detection']])
        }
        
        return {
            'summary': summary,
            'column_details': combined_results
        }

...
class ICD10Encoder:
    ...
    # Common chronic condition groups based on ICD-10 codes
    CHRONIC_CONDITION_GROUPS = {
        ...
        'cancer': ['C00', 'C01', 'C02', 'C03', 'C04', 'C05', 'C06', 'C07', 'C08', 'C09',
                  ...
                  'C93', 'C94', 'C95', 'D00', 'D01', 'D02', 'D03', 'D04', 'D05', 'D06',
                  'D07', 'D09'],
        'depression': ['F32', 'F33'],
        'dementia': ['F01', 'F02', 'F03', 'G30'],
        'stroke': ['I60', 'I61', 'I62', 'I63', 'I65', 'I66', 'I67', 'I69'],
        'obesity': ['E66'],
        'substance_abuse': ['F10', 'F11', 'F12', 'F13', 'F14', 'F15', 'F16', 'F17', 'F18', 'F19']
    }
    ...
    def __init__(self, 
        ...
        # Combine built-in and custom code groups
        self.code_groups = {**self.CHRONIC_CONDITION_GROUPS, **self.custom_code_groups}
        
        # Build chapter lookup for efficient code categorization
        self.chapter_lookup = self._build_chapter_lookup()
        
        logger.info(f"Initialized ICD10Encoder with {len(self.code_groups)} code groups")
    ...
    def _build_chapter_lookup(self) -> Dict[str, str]:
        ...
        for chapter_range, chapter_name in self.ICD10_CHAPTERS.items():
            ...
            # Handle special case for O chapter which ends with 'A'
            if end.endswith('A'):
                end = end[:-1] + 'Z'
            ...
            # Populate lookup for each letter in the range
            ...
            while ord(current_letter) <= ord(end_letter):
                if current_letter == start_letter and current_letter == end_letter:
                    # Both start and end have the same letter
    ...
    def normalize_icd10(self, code: str) -> str:
        ...
        # Remove dots if specified
        if not self.include_dot:
            normalized = normalized.replace(".", "")
        
        # Validate basic ICD-10 format (letter followed by numbers)
        if not re.match(r'^[A-Z]\d+(\.\d+)?$', normalized):
        ...
        if self.max_code_length and len(normalized) > self.max_code_length:
            # Preserve the dot if it's within the max length
    ...
    def get_chapter(self, code: str) -> Optional[str]:
        ...
        # Extract the first three characters (letter + 2 digits)
        prefix = normalized[:3] if len(normalized) >= 3 else normalized
        
        # Remove dot if present
        prefix = prefix.replace('.', '')
        
        # Ensure we have a letter and at least 2 digits
        if len(prefix) < 3:
    ...
    def get_code_group(self, code: str) -> List[str]:
        ...
        for group_name, code_list in self.code_groups.items():
            # Check if the code starts with any of the codes in the group
            if any(normalized.startswith(self.normalize_icd10(c)) for c in code_list):
                matching_groups.append(group_name)
        
        return matching_groups
    
    def encode_codes_binary(self, 
                           codes: List[str], 
                           level: str = 'chapter') -> Dict[str, int]:
        """
        ...
        if level == 'chapter':
            # Encode at chapter level
            for chapter_range, chapter_name in self.ICD10_CHAPTERS.items():
                feature_name = f"icd10_chapter_{chapter_range}"
                features[feature_name] = 0
            
            # Set features based on codes
            for code in codes:
            ...
            # Encode at 3-character category level (e.g., I10)
            ...
            # Encode using predefined condition groups
            ...
            # Use custom encoding logic if provided
            if not self.custom_code_groups:
                logger.warning("No custom code groups defined for 'custom' level encoding")
                return {}
            
            # Initialize features for all custom groups
    ...
    def encode_codes_count(self, 
        ...
        if level == 'chapter':
            # Initialize counts for all chapters
            ...
            # Count codes by 3-character category
            ...
            # Initialize counts for all custom groups
            for group_name in self.custom_code_groups.keys():
                feature_name = f"icd10_custom_{group_name}_count"
                features[feature_name] = 0
            
            # Count codes by custom group
            for code in codes:
                normalized = self.normalize_icd10(code)
                if not normalized:
                    continue
    ...
    def transform(self, 
                 ...
                 encoding_level: str = 'chapter') -> pd.DataFrame:
        """
        ...
        # If patient_id_column is provided, group codes by patient
        if patient_id_column:
            # Group codes by patient
            patient_codes = df.groupby(patient_id_column)[code_column].apply(list).to_dict()
            
            # Encode each patient's codes
            patient_features = {}
            ...
            # Ensure index name matches patient_id_column
            ...
            for _, row in df.iterrows():
                ...
                # Handle single code (convert to list)

...
class TemporalFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, 
        ...
        valid_aggs = ['mean', 'min', 'max', 'median', 'std', 'var', 'count', 'sum', 'range', 'iqr', 'slope']
        for agg in self.aggregation_functions:
            if agg not in valid_aggs:
                raise ValueError(f"Unsupported aggregation function: {agg}. Supported functions: {valid_aggs}")
        
        logger.info(f"Initialized TemporalFeatureExtractor with window sizes: {window_sizes}")
    ...
    def _preprocess_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.reference_time_column and not pd.api.types.is_datetime64_any_dtype(df_copy[self.reference_time_column]):
            df_copy[self.reference_time_column] = pd.to_datetime(df_copy[self.reference_time_column])
        ...
        if self.value_column and self.handle_missing != 'drop':
            if self.handle_missing == 'interpolate':
                ...
                df_copy[self.value_column] = df_copy.groupby(self.patient_id_column)[self.value_column].transform(
    ...
    def _extract_window_features(self, 
                               ...
                               reference_time: pd.Timestamp) -> Dict[str, float]:
        """
        ...
        for window_size in self.window_sizes:
            ...
            for agg_func in self.aggregation_functions:
                feature_name = f"{self.value_column}_{window_size}d_{agg_func}"
                features[feature_name] = self._compute_aggregation(window_data[self.value_column], agg_func)
            ...
            if self.include_trends and len(window_data) >= 2:
                ...
                features[f"{self.value_column}_{window_size}d_time_since_first"] = (reference_time - first_time).total_seconds() / 86400  # days
                features[f"{self.value_column}_{window_size}d_time_since_last"] = (reference_time - last_time).total_seconds() / 86400  # days
                
                # Calculate trend (slope)
                if len(window_data) >= 3:
                    # Convert timestamps to numeric (days from start)
                    window_data = window_data.copy()
                    window_data['days_from_start'] = (window_data[self.time_column] - window_data[self.time_column].min()).dt.total_seconds() / 86400
                    
                    # Calculate slope
                    slope, intercept, r_value, p_value, std_err = stats.linregress(
                        window_data['days_from_start'], 
                        window_data[self.value_column]
                    )
                    
                    features[f"{self.value_column}_{window_size}d_slope"] = slope
                    features[f"{self.value_column}_{window_size}d_r_squared"] = r_value ** 2
    ...
    def _extract_frequency_features(self, 
        ...
        freqs, psd = signal.welch(daily_data.dropna().values, fs=1.0, nperseg=min(len(daily_data), 10))
        ...
        if len(freqs) > 0 and len(psd) > 0:
            ...
            features[f"{self.value_column}_dominant_freq_power"] = psd[dominant_freq_idx]
        
        return features
    
    def _extract_event_features(self, 
                              patient_data: pd.DataFrame, 
                              reference_time: pd.Timestamp) -> Dict[str, float]:
        """
        ...
        for window_size in self.window_sizes:
            ...
            if event_count > 0:
                ...
                features[f"days_since_last_event_{window_size}d"] = (reference_time - most_recent).total_seconds() / 86400
                ...
                if event_count > 1:
                    ...
                    features[f"mean_days_between_events_{window_size}d"] = np.mean(time_diffs)
                    features[f"std_days_between_events_{window_size}d"] = np.std(time_diffs) if len(time_diffs) > 1 else 0
                    features[f"max_days_between_events_{window_size}d"] = np.max(time_diffs)
                    features[f"min_days_between_events_{window_size}d"] = np.min(time_diffs)
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for patient_id, patient_data in patient_groups:
            ...
            for reference_time in reference_times:
                ...
                if self.value_column:
                    window_features = self._extract_window_features(patient_data, reference_time)
                    patient_features.update(window_features)
                    
                    # Extract frequency features
                    freq_features = self._extract_frequency_features(patient_data, reference_time)
                    patient_features.update(freq_features)
                
                # Extract event-based features
                event_features = self._extract_event_features(patient_data, reference_time)
        ...
        if self.reference_time_column:
            result = result.set_index([self.patient_id_column, self.reference_time_column])
        else:
...
class MultiVariateTemporalFeatures:
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized MultiVariateTemporalFeatures with {len(window_sizes)} window sizes")
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_columns = [self.time_column, self.patient_id_column, self.variable_column, self.value_column]
        if self.reference_time_column:
            required_columns.append(self.reference_time_column)
        ...
        for variable in variables:
            ...
            for col in variable_features.columns:
                ...
                elif col.startswith('event_count') or col.startswith('days_since_last_event'):

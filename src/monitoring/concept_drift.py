...
class ConceptDriftDetector:
    ...
    def set_reference_data(self, 
        ...
        logger.info(f"Set reference data with {len(X)} samples and {len(self.feature_names)} features")
    ...
    def _calculate_reference_performance(self):
        ...
        for metric in self.performance_metrics:
            if metric == 'auc':
                ...
                try:
                    self.reference_performance['average_precision'] = average_precision_score(
                        self.reference_y, self.reference_predictions
                    )
                except:
                    logger.warning("Could not calculate average precision on reference data")
    ...
    def detect_drift(self, 
                   ...
                   y: Optional[pd.Series] = None,
                   predictions: Optional[pd.Series] = None,
                   timestamps: Optional[pd.Series] = None) -> bool:
        """
        ...
        for method in self.methods:
            ...
            if method_result.get('drift_detected', False):
                ...
                self.drift_score = max(self.drift_score, method_result.get('drift_score', 0.0))
    ...
    def _detect_drift_js_divergence(self, X, y=None, predictions=None, timestamps=None):
        ...
        for feature in self.feature_names:
            ...
            if feature not in self.categorical_features:
                ...
                ref_hist = np.array([np.mean(ref_values == cat) for cat in all_categories])
                new_hist = np.array([np.mean(new_values == cat) for cat in all_categories])
    ...
    def _detect_drift_hellinger(self, X, y=None, predictions=None, timestamps=None):
        ...
        for feature in self.feature_names:
            ...
            hellinger_dist = np.sqrt(0.5 * np.sum((np.sqrt(ref_hist) - np.sqrt(new_hist)) ** 2))
    ...
    def _detect_drift_psi(self, X, y=None, predictions=None, timestamps=None):
        ...
        for feature in self.feature_names:
            ...
            if feature not in self.categorical_features:
                ...
                ref_hist = np.array([np.sum(ref_values == cat) for cat in all_categories])
                new_hist = np.array([np.sum(new_values == cat) for cat in all_categories])
            ...
            if psi > 0.2:  # Threshold for PSI
                result['drift_features'].append(feature)
                result['drift_score'] = max(result['drift_score'], psi / 2)  # Normalize to [0, 1]
    ...
    def _detect_drift_adwin(self, X, y=None, predictions=None, timestamps=None):
        ...
        for value in values:
            ...
            self.adwin_variance = (self.adwin_n * self.adwin_variance + (value - mean) ** 2) / (self.adwin_n + 1)
            
            # Check for drift
            if self.adwin_n > 10:
                # Calculate threshold
                delta = 0.002
                epsilon = np.sqrt(2 * self.adwin_variance * np.log(1 / delta) / self.adwin_n)
                
                # Detect drift if mean change exceeds threshold
                if np.abs(mean - (self.adwin_sum - value) / (self.adwin_n - 1)) > epsilon:
                    result['drift_detected'] = True
    ...
    def _detect_drift_ddm(self, X, y=None, predictions=None, timestamps=None):
        ...
        for error in errors:
            ...
            error_rate = error
            self.ddm_p = self.ddm_p + (error_rate - self.ddm_p) / self.adwin_n if self.adwin_n > 0 else error_rate
            self.ddm_s = np.sqrt(self.ddm_p * (1 - self.ddm_p) / self.adwin_n) if self.adwin_n > 0 else 0
            ...
            if self.ddm_p + self.ddm_s > self.ddm_p_min + self.ddm_warning_level * self.ddm_s_min:
            ...
            if self.ddm_p + self.ddm_s > self.ddm_p_min + self.ddm_drift_level * self.ddm_s_min:
                result['drift_detected'] = True
                result['drift_score'] = min(1.0, (self.ddm_p + self.ddm_s - self.ddm_p_min) / (self.ddm_drift_level * self.ddm_s_min))
                break
    ...
    def _detect_drift_eddm(self, X, y=None, predictions=None, timestamps=None):
        ...
        if len(error_indices) > 1:
            ...
            if eddm_stat < 0.90 * self.eddm_max:
                result['drift_detected'] = True
                result['drift_score'] = min(1.0, (self.eddm_max - eddm_stat) / (0.1 * self.eddm_max))
            
            # Store statistics
            result['statistics'] = {
                'eddm_stat': eddm_stat,
                'eddm_max': self.eddm_max
            }
        
        return result
    
    def _detect_drift_isolation_forest(self, X, y=None, predictions=None, timestamps=None):
        """
    ...
    def _detect_drift_performance_drop(self, X, y=None, predictions=None, timestamps=None):
        """
        ...
        for metric in self.performance_metrics:
            if metric == 'auc':
                ...
                try:
                    current_performance['average_precision'] = average_precision_score(y, predictions)
                except:
                    logger.warning("Could not calculate average precision on current data")
    ...
    def plot_drift_statistics(self, feature=None, method=None, ax=None):
        ...
        if method is not None:
            ...
            if method in ['ks_test', 'chi_squared', 'js_divergence', 'hellinger', 'psi']:
                # Plot feature-based statistics
                if feature is not None:
                    if feature not in method_stats['statistics']:
                        raise ValueError(f"Feature {feature} not found in {method} statistics")
                    ...
                    if 'p_value' in feature_stats:
                        ...
                        ax.axhline(y=self.sensitivity, color='r', linestyle='--', label=f'Threshold ({self.sensitivity})')
                        ...
                        ax.bar(['Hellinger Distance'], [feature_stats['hellinger_distance']])
                    ...
                    if not features:
                        ax.text(0.5, 0.5, f"No feature statistics available for {method}",
                               ha='center', va='center', transform=ax.transAxes)
                    else:
                        # Determine statistic to plot
                        stat_key = None
                        for key in ['p_value', 'js_divergence', 'hellinger_distance', 'psi']:
                            if key in method_stats['statistics'][features[0]]:
                                stat_key = key
                                break
                        
                        if stat_key:
                            values = [method_stats['statistics'][f].get(stat_key, 0) for f in features]
                            ax.bar(features, values)
                            
                            # Add threshold line
                            threshold = 0.05 if stat_key == 'p_value' else 0.1 if stat_key == 'js_divergence' else 0.2
                ...
                if method == 'adwin':
                    ax.bar(['Mean', 'Variance'], [stats['adwin_mean'], stats['adwin_variance']])
                    ax.set_title('ADWIN Statistics')
                elif method == 'page_hinkley':
                    ax.bar(['Mean', 'Sum'], [stats['ph_mean'], stats['ph_sum']])
                    ax.axhline(y=self.ph_threshold, color='r', linestyle='--', label=f'Threshold ({self.ph_threshold})')
                    ...
                    ax.bar(['Mean Score', 'Max Score', 'Threshold', 'Anomaly Rate'],
                          [stats['mean_anomaly_score'], stats['max_anomaly_score'],
                           stats['anomaly_threshold'], stats['anomaly_rate']])
                    ax.set_title(f'{method} Statistics')
                elif method == 'performance_drop':
                    # Plot performance comparison
                    metrics = list(set(stats['reference_performance'].keys()) & set(stats['current_performance'].keys()))
                    
                    if metrics:
                        ref_values = [stats['reference_performance'][m] for m in metrics]
            ...
            drift_scores = [self.drift_statistics[m].get('drift_score', 0) for m in methods]
            ...
            if 'drift_features' in stats:
                for feature in stats.get('drift_features', []):
                    ...
                    if method in stats['statistics'] and feature in stats['statistics']:
                        report['feature_statistics'][feature][method] = stats['statistics'][feature]

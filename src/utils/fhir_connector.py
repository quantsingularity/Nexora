...
class FHIRConnector:
    ...
    RESOURCE_TYPES = [
        ...
        'Encounter', 'Immunization', 'CarePlan', 'Goal', 'Device', 'Organization',
        'Practitioner', 'Location', 'Claim', 'ExplanationOfBenefit'
    ]
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized FHIR connector for {base_url} with {auth_type} authentication")
    ...
    def search(self, 
        ...
        search_params['_count'] = min(self.page_size, max_count) if max_count else self.page_size
    ...
    def patients_to_dataframe(self, patients: List[Dict]) -> pd.DataFrame:
        ...
        for patient in patients:
            ...
            email = next((t.get('value', '') for t in telecom if t.get('system') == 'email'), '')
            
            # Extract identifiers
            identifiers = patient.get('identifier', [])
            mrn = next((i.get('value', '') for i in identifiers if i.get('type', {}).get('coding', [{}])[0].get('code') == 'MR'), '')
            ssn = next((i.get('value', '') for i in identifiers if i.get('system') == 'http://hl7.org/fhir/sid/us-ssn'), '')
            ...
            if 'valueQuantity' in obs:
                ...
                for component in obs['component']:
                    ...
                    if 'valueQuantity' in component:
                        ...
                        comp_value_coding = component['valueCodeableConcept'].get('coding', [{}])[0]
    ...
    def procedures_to_dataframe(self, procedures: List[Dict]) -> pd.DataFrame:
        ...
        for procedure in procedures:
            ...
            patient_id = procedure.get('subject', {}).get('reference', '').replace('Patient/', '')
            ...
            # Extract performer
            performer = procedure.get('performer', [{}])[0].get('actor', {}).get('display', '')
    ...
    def medications_to_dataframe(self, medications: List[Dict]) -> pd.DataFrame:
        ...
        for med in medications:
            # Extract basic information
            med_id = med.get('id', '')
            patient_id = med.get('subject', {}).get('reference', '').replace('Patient/', '')
            
            # Extract dates
            authored_date = med.get('authoredOn', '')
            
            # Extract medication
            medication_reference = med.get('medicationReference', {}).get('reference', '')
            medication_display = med.get('medicationReference', {}).get('display', '')
            
            if not medication_display and 'medicationCodeableConcept' in med:
                medication_coding = med['medicationCodeableConcept'].get('coding', [{}])[0]
    ...
    def dataframe_to_patients(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            ...
            patient = {
                ...
                'name': [
                    {
                        ...
                        'given': row.get('given_name', '').split() if row.get('given_name') else []
                    }
                ],
                ...
                'address': [
                    {
                        'line': [row.get('address_line', '')] if row.get('address_line') else [],
                        'city': row.get('city', ''),
                        'state': row.get('state', ''),
    ...
    def dataframe_to_observations(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            ...
            if value_type == 'quantity':
                ...
                observation['valueBoolean'] = bool(value) if value and not pd.isna(value) else None
            elif value_type == 'integer':
                observation['valueInteger'] = int(value) if value and not pd.isna(value) else None
    ...
    def bulk_export(self, 
        ...
        for resource_type in resource_types:
            ...
            logger.info(f"Exported {len(resources)} {resource_type} resources to {file_path}")
        
        return result
    
    def bulk_import(self, file_path: str, update_if_exists: bool = False) -> Dict[str, int]:
        """

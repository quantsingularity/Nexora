...
class HealthcareMetrics:
    ...
    def calculate_length_of_stay(self, df: pd.DataFrame) -> pd.Series:
        ...
        if self.admission_date_column not in df.columns:
            raise ValueError(f"Admission date column '{self.admission_date_column}' not found")
        
        if self.discharge_date_column not in df.columns:
            raise ValueError(f"Discharge date column '{self.discharge_date_column}' not found")
    ...
    def calculate_readmission_rate(self, 
                                 df: pd.DataFrame, 
                                 window_days: int = 30,
                                 condition_column: Optional[str] = None,
                                 condition_value: Optional[str] = None) -> float:
        """
        ...
        # Ensure dates are datetime
        filtered_df[self.discharge_date_column] = pd.to_datetime(filtered_df[self.discharge_date_column])
        filtered_df[self.admission_date_column] = pd.to_datetime(filtered_df[self.admission_date_column])
        
        # Sort by patient ID and admission date
        filtered_df = filtered_df.sort_values([self.patient_id_column, self.admission_date_column])
        ...
        for patient_id, patient_df in filtered_df.groupby(self.patient_id_column):
            ...
            encounters = patient_df.sort_values(self.admission_date_column).reset_index(drop=True)
            ...
            for i in range(len(encounters) - 1):
                ...
                days_between = (next_admission_date - discharge_date).total_seconds() / (24 * 3600)
                
                # Check if readmission is within window
                if 0 <= days_between <= window_days:
                    readmissions += 1
                    filtered_df.loc[encounters.iloc[i+1].name, 'is_readmission'] = True
                
                total_discharges += 1
        
        # Calculate readmission rate
        readmission_rate = readmissions / total_discharges if total_discharges > 0 else 0
        
        logger.info(f"Calculated {window_days}-day readmission rate: {readmission_rate:.4f}")
    ...
    def calculate_mortality_index(self, df: pd.DataFrame) -> float:
        ...
        mortality_index = observed_mortality / expected_mortality if expected_mortality > 0 else np.nan
        
        logger.info(f"Calculated mortality index: {mortality_index:.4f}")
        return mortality_index
    
    def calculate_complication_rate(self, 
                                  df: pd.DataFrame,
                                  complication_type: Optional[str] = None) -> float:
        """
        ...
        if self.complication_column not in df.columns:
            raise ValueError(f"Complication column '{self.complication_column}' not found")
        ...
        if complication_type:
            if isinstance(df[self.complication_column].iloc[0], str):
                ...
                complication_rate = (df[self.complication_column] == complication_type).mean()
            else:
    ...
    def calculate_standardized_mortality_ratio(self, 
                                            df: pd.DataFrame,
                                            groupby_column: Optional[str] = None) -> Union[float, pd.Series]:
        """
        ...
        if groupby_column:
            ...
            if observed > 0:
                ...
                ci_lower = 0
                ci_upper = 3.69 / expected if expected > 0 else np.nan  # 95% CI for 0 observed events
            
            logger.info(f"Calculated SMR: {smr:.4f} (95% CI: {ci_lower:.4f}-{ci_upper:.4f})")
    ...
    def calculate_risk_adjusted_readmission(self, 
                                         df: pd.DataFrame,
                                         window_days: int = 30,
                                         groupby_column: Optional[str] = None) -> Union[float, pd.Series]:
        """
        ...
        if self.readmission_column not in df.columns:
            logger.info(f"Readmission column not found, calculating {window_days}-day readmissions")
            ...
            for patient_id, patient_df in df.groupby(self.patient_id_column):
                ...
                # Check each discharge
                for i in range(len(encounters) - 1):
                    discharge_date = pd.to_datetime(encounters.iloc[i][self.discharge_date_column])
                    next_admission_date = pd.to_datetime(encounters.iloc[i+1][self.admission_date_column])
        ...
        if groupby_column:
            ...
            logger.info(f"Calculated risk-adjusted readmission rate: {risk_adjusted:.4f}")
            return risk_adjusted
    ...
    def evaluate_clinical_model(self, 
                             df: pd.DataFrame,
                             outcome_column: str,
                             prediction_column: Optional[str] = None) -> Dict:
        """
        ...
        try:
            prob_true, prob_pred = calibration_curve(df[outcome_column], df[prediction_column], n_bins=10)
            calibration_slope, calibration_intercept = np.polyfit(prob_pred, prob_true, 1)
        except:
        ...
        logger.info(f"Model evaluation results: AUC={auc:.4f}, AP={ap:.4f}, Brier={brier:.4f}")
        return results
    ...
    def plot_model_performance(self, 
        ...
        if len(prob_true) > 0 and len(prob_pred) > 0:
            ...
            ax1.text(0.1, 0.9, f'Slope: {slope:.2f}\nIntercept: {intercept:.2f}',
    ...
    def calculate_quality_metrics(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            metrics['readmission_30day'] = self.calculate_readmission_rate(df, window_days=30)
            metrics['readmission_7day'] = self.calculate_readmission_rate(df, window_days=7)
            
            # Calculate risk-adjusted readmission if expected column is available
            if self.expected_column in df.columns:
                metrics['risk_adjusted_readmission'] = self.calculate_risk_adjusted_readmission(df)
        except Exception as e:
    ...
    def calculate_stratified_metrics(self, 
                                  df: pd.DataFrame, 
                                  stratify_column: str) -> Dict:
        """
        ...
        for stratum in strata:
            ...
            if len(stratum_df) < 10:
                logger.warning(f"Skipping stratum '{stratum}' with only {len(stratum_df)} samples")
                continue
    ...
    def calculate_observed_expected_ratio(self, 
                                       df: pd.DataFrame,
                                       outcome_column: str,
                                       expected_column: Optional[str] = None) -> float:
        """
    ...
    def calculate_risk_adjusted_rate(self, 
                                  df: pd.DataFrame,
                                  outcome_column: str,
                                  expected_column: Optional[str] = None,
                                  reference_rate: Optional[float] = None) -> float:
        """
        ...
        # Calculate O/E ratio
        oe_ratio = self.calculate_observed_expected_ratio(df, outcome_column, expected_column)
        ...
        logger.info(f"Calculated risk-adjusted rate for {outcome_column}: {adjusted_rate:.4f}")
        return adjusted_rate
    
    def calculate_excess_events(self, 
                             df: pd.DataFrame,
                             outcome_column: str,
                             expected_column: Optional[str] = None) -> int:
        """
    ...
    def plot_quality_dashboard(self, 
        ...
        ax2 = axes[0, 1]
        readmission_metrics = {k: v for k, v in metrics.items() if 'readmission' in k.lower()}
        if readmission_metrics:
            ...
            ax2.text(0.5, 0.5, 'No readmission metrics available', ha='center', va='center')
            ax2.set_title('Readmission Metrics')
        
        # Plot mortality metrics
        ax3 = axes[1, 0]
        mortality_metrics = {k: v for k, v in metrics.items() if 'mortality' in k.lower() or k == 'smr'}
        if mortality_metrics:
            ...
            ax3.text(0.5, 0.5, 'No mortality metrics available', ha='center', va='center')
            ax3.set_title('Mortality Metrics')
        
        # Plot other metrics
        ax4 = axes[1, 1]
        other_metrics = {k: v for k, v in metrics.items() 
                        if 'los' not in k.lower() and 'readmission' not in k.lower() 
                        and 'mortality' not in k.lower() and k != 'smr'}
        if other_metrics:
    ...
    def plot_stratified_metrics(self, 

...
def run_tests(output_dir='test_results'):
    ...
    report = {
        ...
        'details': {
            'failures': [{'test': test, 'message': message} for test, message in result.failures],
            'errors': [{'test': test, 'message': message} for test, message in result.errors],
            'skipped': [{'test': test, 'message': message} for test, message in result.skipped]
        }
    }
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run tests for HIPAA-compliant readmission prediction pipeline')
    parser.add_argument('--output-dir', type=str, default='test_results', help='Directory for test results')

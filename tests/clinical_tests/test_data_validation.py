...
class TestDataValidation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data['admission_date'] = pd.to_datetime(self.sample_data['admission_date'])
        self.sample_data['discharge_date'] = pd.to_datetime(self.sample_data['discharge_date'])
        ...
        self.schema = {
            ...
            'diagnosis_code': {'type': 'string', 'required': True, 'pattern': r'^[A-Z][0-9]{2}(\.[0-9]{1,2})?$'},
        ...
        self.relationships = [
            {'type': 'temporal', 'first': 'admission_date', 'second': 'discharge_date', 'relation': '<='},
            {'type': 'logical', 'condition': 'mortality == 1', 'implication': 'readmission == 0'}
        ]
    ...
    def test_validate_relationships(self):
        ...
        # Test valid data
        result = self.validator.validate_relationships(self.sample_data, self.relationships)
        self.assertTrue(result['valid'])
        self.assertEqual(len(result['errors']), 0)
        
        # Test invalid data - temporal relationship violation
        invalid_data = self.sample_data.copy()
        invalid_data.loc[0, 'discharge_date'] = invalid_data.loc[0, 'admission_date'] - timedelta(days=1)
        ...
        invalid_data.loc[3, 'readmission'] = 1  # Logical contradiction: dead patients can't be readmitted
        result = self.validator.validate_relationships(invalid_data, self.relationships)
        self.assertFalse(result['valid'])
        self.assertGreater(len(result['errors']), 0)
    ...
    def test_detect_outliers(self):
        ...
        result = self.validator.detect_outliers(self.sample_data, ['age', 'lab_value', 'medication_count'])
    ...
    def test_validate_icd10_codes(self):
        ...
        valid_codes = pd.Series(['A01.0', 'B20', 'C34.90', 'D61.810', 'E11.9', 'I25.10', 'J44.9', 'K21.0', 'Z99.2'])
    ...
    def test_validate_consistency(self):
        ...
        consistency_rules = [
            {'condition': 'age >= 65', 'expected': 'medication_count >= 3', 'name': 'elderly_medication'},
            {'condition': 'mortality == 1', 'expected': 'age > 70', 'name': 'mortality_age_correlation'}
        ]
        
        # Test valid data
        result = self.validator.validate_consistency(self.sample_data, consistency_rules)
    ...
    def test_integration_with_fhir(self):
        ...
        observations_df = mock_connector.observations_to_dataframe(fhir_data['Observation'])

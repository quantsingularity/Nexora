...
class TestFHIRIngest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_condition = {
            ...
            'meta': {
                'versionId': '1',
                'lastUpdated': '2023-05-15T10:35:00Z'
            },
            'clinicalStatus': {
                'coding': [
                    {
        ...
        self.sample_medication_request = {
            ...
            'meta': {
                'versionId': '1',
                'lastUpdated': '2023-05-15T10:40:00Z'
            },
            'status': 'active',
            'intent': 'order',
    ...
    def test_patient_conversion(self):
        ...
        essential_columns = ['patient_id', 'family_name', 'given_name', 'gender', 'birth_date']
    ...
    def test_observation_conversion(self):
        ...
        observations_df = self.mock_connector.observations_to_dataframe([self.sample_observation])
        ...
        # Check essential columns
        essential_columns = ['observation_id', 'patient_id', 'code', 'display', 'value', 'unit', 'date']
        for column in essential_columns:
    ...
    def test_condition_conversion(self):
        ...
        conditions_df = self.mock_connector.conditions_to_dataframe([self.sample_condition])
        ...
        # Check essential columns
        essential_columns = ['condition_id', 'patient_id', 'code', 'display', 'onset_date', 'clinical_status']
        for column in essential_columns:
    ...
    def test_medication_conversion(self):
        ...
        medications_df = self.mock_connector.medications_to_dataframe([self.sample_medication_request])
        ...
        # Check essential columns
        essential_columns = ['medication_id', 'patient_id', 'medication_display', 'status', 'authored_date']
        for column in essential_columns:
            self.assertIn(column, medications_df.columns)
        
        # Check values
        self.assertEqual(medications_df.iloc[0]['medication_id'], 'medication-request-001')
    ...
    def test_data_validation_integration(self):
        ...
        patient_schema = {
            ...
            'gender': {'type': 'category', 'required': True, 'categories': ['male', 'female', 'other', 'unknown']},
...
class MockFHIRConnector:
                ...
                for key, value in params.items():
                    if key == 'name' and resource.get('resourceType') == 'Patient':
                        ...
                        if resource.get('resourceType') == 'Observation':
                            code_value = resource.get('code', {}).get('coding', [{}])[0].get('code', '')
                            if value != code_value:
                                match = False
                                break
    ...
    def dataframe_to_patients(self, df):
        ...
        for _, row in df.iterrows():
            ...
            if row.get('address_line') or row.get('city') or row.get('state') or row.get('postal_code'):
                patient['address'] = [
                    {

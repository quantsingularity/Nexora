describe("Patient Workflow", () => {
  beforeEach(() => {
    cy.intercept("GET", "/api/patients/*", { fixture: "patient.json" }).as(
      "getPatient",
    );
    cy.intercept("GET", "/api/predictions/*", {
  ...
  it("completes full patient workflow", () => {
    ...
    cy.get('[data-testid="clinical-notes"]').should(
      "contain",
    ...
    cy.get('[data-testid="risk-history"]').should(
      "contain",
      "Increased risk due to recent lab results",
    );
  ...
  it("handles error states gracefully", () => {
    ...
    cy.intercept("GET", "/api/patients/*", { statusCode: 500 }).as(
      "getPatientError",
    );
    ...
    // Test network error
    cy.intercept("GET", "/api/predictions/*", { forceNetworkError: true }).as(
      "getPredictionsError",
    );
    cy.visit("/patients/123");
    cy.get('[data-testid="error-message"]').should("be.visible");
  ...
  it("maintains data consistency", () => {
    ...
    // Verify audit trail
    cy.get('[data-testid="audit-trail"]').should(
      "contain",
